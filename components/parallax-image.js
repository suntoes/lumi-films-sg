...
const ParallaxImage = () => {
  ...
  const getEaseOut = (i, axis, percentage) => 
    (i * (0.1 * ((axis === 'x' ? xMultiplier : yMultiplier) * i))) * ((percentage || 0) * parallaxMultiplier )
  ...
  useEffect(() => {
    ...
    const handleParallax = (e) => {
      ...
      const { clientHeight: boxHeight, clientWidth: boxWidth, offsetTop, offsetLeft } = boxRef.current
      const { clientHeight, clientWidth } = document.documentElement
      const { pageYOffset: scrollY } = window

      const boxOriginX = offsetLeft + (boxWidth/2)
      const xMultiplier =  mouseX < boxOriginX 
      ...
      const yMultiplier = screenMidPoint < boxOriginY 
        ? ((screenMidPoint-boxOriginY)/boxOriginY) * -3
        : ((screenMidPoint-boxOriginY)/boxOriginY) * -1

      for(let i in imageRefs) {
        if(!imageRefs[i]) return
        const transformVal = `translateX(${getEaseOut(Number(i), 'x', xMultiplier)}px) translateY(${getEaseOut(Number(i), 'y', yMultiplier)}px)`
